...
export default class ActorSheetEd extends ActorSheet {
   ...
   */
    ...
    _onChangeItemStatus( event ) {
      ...
      const isPiecemealArmor = item.type === "armor" ? item.system.piecemealArmor.selector : 0;
      ...
      // items: collection of items per type (weapons, shields, armorItems or equipment)
      // status: the new status of the item to check for
      // specialSelection: if the item is a special selection 
      ...
      if ( item.type === "weapon" ) {
        ...
        if ( newItemStatus === 3 ) {
          ...
          if ( isOneHandedWeapon && !isBow && !isCrossBow ) {
          ...
          // two handed weapons can only be equipped in two hands same applies to bows and Crossbows independent of their size
          else if ( isTwoHandedWeapon || isCrossBow ) {
              newItemStatus = 6;
              updateItemStatus( weapons, 4 );
              updateItemStatus( weapons, 5 );
              updateItemStatus( weapons, 6 );
              ...
              shields.forEach( shield => {
                if ( shield.system.itemStatus.value === 5 && !bowUsage ) {	
                  item.update( { "system.itemStatus.value": 2 } );
                }
              } );
            } else {
        ...
        else if ( newItemStatus === 5 && isOneHandedWeapon ) {
            updateItemStatus( weapons, 5 );
            updateItemStatus( weapons, 6 );
            updateItemStatus( shields, 5 );
          } 
